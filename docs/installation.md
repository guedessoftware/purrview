...
O padrão instala em `~/.local/opt/impage` (caminho legado preservado para atualização), cria somente links próprios em `~/.local` e registra um
...
use `scripts/install-impage.sh --user --verify` ou combine `--verify` com `--upgrade`.
...
scripts/install-impage.sh --uninstall
sudo scripts/install-impage.sh --system
```

`--system` é a única forma de escolher `/opt/impage` e `/usr/local`; requer autorização explícita.
...
O log fica em `${XDG_CACHE_HOME:-~/.cache}/impage/install.log`.

O comando instalado fica em `~/.local/opt/impage/bin/purrview`, com um link conveniente em
...
Não execute `./build/release/impage` nem `cmake --install build/release` dentro do pacote bootstrap:
esse diretório pertence exclusivamente ao fluxo de build a partir do fonte descrito abaixo. O
bootstrap compila temporariamente em `~/.cache/impage/` e já realiza a instalação ao terminar.
