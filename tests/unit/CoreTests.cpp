...
void testLayoutOneByOne() {
    purrview::core::DocumentModel document;
    document.grid().setRows(1);
    document.grid().setColumns(1);
    document.page().setMarginsMm(10.0, 10.0, 10.0, 10.0);

    const purrview::core::PageLayout layout = purrview::core::LayoutEngine().calculate(document);
...
void testLandscapeLayout() {
    purrview::core::DocumentModel document;
    document.page().setOrientation(purrview::core::PageModel::Orientation::Landscape);
...
void testPaperSizes() {
    purrview::core::PageModel page;
    check(page.paperSize() == purrview::core::PageModel::PaperSize::A4,
...
void testInvalidLayout() {
    ...
    check(!purrview::core::LayoutEngine().calculate(document).isValid(),
...
void testImagePlacement() {
    const purrview::core::ImagePlacementEngine engine;
    const QRectF squareCell(0.0, 0.0, 100.0, 100.0);

    const auto fit =
        engine.calculate(QSizeF(200.0, 100.0), squareCell, purrview::core::PlacementMode::Fit);
    ...
    const auto fill =
        engine.calculate(QSizeF(200.0, 100.0), squareCell, purrview::core::PlacementMode::Fill);
    ...
    const auto portraitFit =
        engine.calculate(QSizeF(100.0, 200.0), squareCell, purrview::core::PlacementMode::Fit);
    ...
    const auto stretch =
        engine.calculate(QSizeF(200.0, 100.0), squareCell, purrview::core::PlacementMode::Stretch);
...
void testUnits() {
    checkNear(purrview::core::units::millimetersToPixels(25.4, 300.0), 300.0,
              "25.4 mm is 300 px at 300 dpi");
    checkNear(purrview::core::units::pixelsToMillimeters(600.0, 600.0), 25.4,
              "600 px is 25.4 mm at 600 dpi");
}

void testImageResourceLimits() {
    check(purrview::core::isImageSizeWithinLimits(QSize(8192, 8192)),
          "64-megapixel boundary is accepted");
    check(!purrview::core::isImageSizeWithinLimits(QSize(8193, 8192)),
          "images above the pixel ceiling are rejected");
    check(!purrview::core::isImageSizeWithinLimits(QSize()),
          "invalid image dimensions are rejected");
    check(!purrview::core::isImageSizeWithinLimits(QSize(100, 100), 0),
          "invalid resource ceilings are rejected");
}
...
void testPageRenderer() {
    ...
    purrview::core::PageRenderer renderer;
    renderer.setMaximumCacheCostBytes(100'000);
    const bool rendered = renderer.render(painter, document, page.rect(),
                                          purrview::core::PageRenderer::Purpose::Print);
    painter.end();
    ...
    check(renderer.currentCacheCostBytes() <= renderer.maximumCacheCostBytes(),
          "renderer cache remains inside its byte budget");

    page.fill(Qt::transparent);
    painter.begin(&page);
    const bool secondPageRendered = renderer.render(
        painter, document, page.rect(), purrview::core::PageRenderer::Purpose::Print, 1);
    painter.end();
    ...
    purrview::core::DocumentModel rotatedDocument;
